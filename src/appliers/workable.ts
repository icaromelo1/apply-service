...
export async function applyWorkable(applicationId: number, job: Job): Promise<ApplyOutcome> {
  ...
  try {
    ...
    const nome =
      (await preencher(page, ['input[name="firstname"]', 'input[id="firstname"]'], candidato.nome)) &&
      (await preencher(page, ['input[name="lastname"]', 'input[id="lastname"]'], candidato.sobrenome));
    ...
    if (!nome || !email) {
      const shot = await saveScreenshot(page, applicationId);
      return { status: "needs_review", note: `form fora do padrão workable — ${shot}` };
    }
